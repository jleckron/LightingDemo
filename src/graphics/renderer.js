    ...
    */
    ...
    render() {
        ...
        for (var i = 0; i < this.scene.geometries.length; i++) {
            ...
            geometry.shader.setUniform("u_CameraPosition", this.camera.cameraPosition.elements);


            if(this.scene.light != null) {

              var rot = new Matrix4();
              var trans1 = new Matrix4();
              var trans2 = new Matrix4();
              rot.setRotate(0.001, 0, 0, 1);
              trans1.setTranslate(-16, 0, 0);
              trans2.setTranslate(16, 0 ,0);

              this.scene.light.pos = trans1.multiplyVector3(this.scene.light.pos);
              this.scene.light.pos = rot.multiplyVector3(this.scene.light.pos);
              this.scene.light.pos = trans2.multiplyVector3(this.scene.light.pos);
                ...
                geometry.shader.setUniform("u_SpecularColor", this.scene.light.spec);
    ...
    initGLSLBuffers() {
        ...
        if (!attributeBuffer || !indexBuffer || !vertexTexCoordBuffer) {
